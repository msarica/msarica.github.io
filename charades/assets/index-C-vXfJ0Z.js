(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();function e(e){if(typeof e==`string`)return e.trim();if(e&&typeof e==`object`&&`text`in e){let t=e.text;if(typeof t==`string`)return t.trim()}throw Error(`Invalid word entry: ${JSON.stringify(e)}`)}function t(){let e=`/charades/`;return`${e.endsWith(`/`)?e:`${e}/`}assets/words/`}function n(e){return e.replace(/\.json$/i,``)||`deck`}async function r(){let r=t(),i=`${r}manifest.json`,a=await fetch(i);if(!a.ok)throw Error(`Failed to load manifest (${a.status}): ${i}`);let o=(await a.json()).decks?.[0];if(!o?.file||!o?.name)throw Error(`Manifest must include at least one deck with "name" and "file"`);let s=`${r}${o.file}`,c=await fetch(s);if(!c.ok)throw Error(`Failed to load deck (${c.status}): ${s}`);let l=(await c.json()).words;if(!Array.isArray(l)||l.length===0)throw Error(`Deck must include a non-empty "words" array: ${s}`);let u=l.map(e).filter(e=>e.length>0);if(u.length===0)throw Error(`No valid words after normalization: ${s}`);return{displayName:o.name,id:n(o.file),words:u}}function i(e,t,n){return{deckDisplayName:e,deckId:t,durationSec:n,events:[]}}function a(e){let t=0,n=0;for(let r of e.events)r.outcome===`correct`?t+=1:n+=1;return{correct:t,passed:n}}var o=52,s=128;function c(e,t,n){let r=0,i=i=>{if(!n.isActive())return;let a=i.beta;if(a==null||Number.isNaN(a))return;let c=performance.now();c-r<n.cooldownMs||(a<o?(r=c,e()):a>s&&(r=c,t()))};return window.addEventListener(`deviceorientation`,i,!0),()=>window.removeEventListener(`deviceorientation`,i,!0)}async function l(){let e=DeviceOrientationEvent;if(typeof e.requestPermission==`function`)try{return await e.requestPermission()===`granted`}catch{return!1}return!0}var u=null;function d(){return u||=new AudioContext,u}async function f(){let e=d();e.state===`suspended`&&await e.resume()}function p(){let e=d(),t=e.currentTime,n=e.createGain();n.gain.setValueAtTime(.12,t),n.connect(e.destination);let r=[523.25,659.25,783.99];for(let i=0;i<r.length;i++){let a=e.createOscillator(),o=e.createGain();a.type=`sine`,a.frequency.setValueAtTime(r[i],t),o.gain.setValueAtTime(1e-4,t),o.gain.exponentialRampToValueAtTime(.35,t+.02+i*.04),o.gain.exponentialRampToValueAtTime(1e-4,t+.35+i*.05),a.connect(o),o.connect(n),a.start(t+i*.04),a.stop(t+.45)}}function m(){let e=d(),t=e.currentTime,n=e.createGain();n.gain.setValueAtTime(.085,t),n.connect(e.destination);let r=e.createOscillator(),i=e.createGain();r.type=`triangle`,r.frequency.setValueAtTime(380,t),r.frequency.exponentialRampToValueAtTime(220,t+.1),i.gain.setValueAtTime(1e-4,t),i.gain.exponentialRampToValueAtTime(.45,t+.015),i.gain.exponentialRampToValueAtTime(1e-4,t+.14),r.connect(i),i.connect(n),r.start(t),r.stop(t+.16)}function h(){let e=d(),t=e.currentTime,n=e.createOscillator(),r=e.createGain();n.type=`sine`,n.frequency.setValueAtTime(880,t),r.gain.setValueAtTime(1e-4,t),r.gain.exponentialRampToValueAtTime(.06,t+.01),r.gain.exponentialRampToValueAtTime(1e-4,t+.12),n.connect(r),r.connect(e.destination),n.start(t),n.stop(t+.15)}var g=`charades-duration-sec`,_=550,v=30,y=300;function b(e){let t=[...e];for(let e=t.length-1;e>0;e--){let n=Math.floor(Math.random()*(e+1)),r=t[e];t[e]=t[n],t[n]=r}return t}function x(){let e=sessionStorage.getItem(g),t=e?Number(e):NaN;return Number.isFinite(t)&&t>=v&&t<=y?Math.round(t):90}function S(e){let t=Math.max(0,Math.ceil(e));return`${Math.floor(t/60)}:${(t%60).toString().padStart(2,`0`)}`}var C=class{root;phase=`loading`;errorMessage=``;deck=null;durationSec=x();session=null;playWords=[];playIndex=0;playDurationMs=0;playStartedAt=0;lastTimerCeilSec=null;tiltCleanup=null;raf=0;tiltDenied=!1;reducedMotion=!1;wordEl=null;timeEl=null;lastAdvanceAt=0;lastSubmittedPlayIndex=null;constructor(e){this.root=e,this.reducedMotion=typeof matchMedia<`u`&&matchMedia(`(prefers-reduced-motion: reduce)`).matches}async boot(){try{this.deck=await r(),this.phase=`setup`}catch(e){this.phase=`error`,this.errorMessage=e instanceof Error?e.message:String(e)}this.render()}persistDuration(){sessionStorage.setItem(g,String(this.durationSec))}render(){switch(this.phase){case`loading`:this.renderLoading();break;case`error`:this.renderError();break;case`setup`:this.renderSetup();break;case`play`:this.mountPlay();break;case`ended`:this.renderEnded();break}}renderLoading(){this.root.innerHTML=`
      ...
      <main class="shell">
        <h1 class="title">Charades</h1>
        <p class="status proof--error" role="alert">${w(this.errorMessage)}</p>
      ...
      <main class="shell setup">
        <h1 class="title">Charades</h1>
        <p class="deck-label"><span class="muted">Deck</span> ${w(e.displayName)}</p>
        ...
        <fieldset class="duration-field">
          ...
          <label class="duration-custom">
            ...
            <input type="number" min="${v}" max="${y}" step="5" value="${this.durationSec}" class="duration-input" aria-label="Round length in seconds" />
            <span class="muted">seconds</span>
          </label>
        </fieldset>
        <button type="button" class="btn btn-primary" id="btn-start">Start round</button>
      </main>`;let n=this.root.querySelector(`.chip-row`);n?.addEventListener(`click`,e=>{let t=e.target.closest(`button[data-duration]`);if(!t)return;let r=Number(t.getAttribute(`data-duration`));if(!Number.isFinite(r))return;this.durationSec=r,this.persistDuration();let i=this.root.querySelector(`.duration-input`);i&&(i.value=String(r)),n.querySelectorAll(`.chip`).forEach(e=>{e.classList.toggle(`chip--on`,e===t)})});let r=this.root.querySelector(`.duration-input`);r?.addEventListener(`change`,()=>{let e=Math.round(Number(r.value));Number.isFinite(e)||(e=90),e=Math.min(y,Math.max(v,e)),this.durationSec=e,r.value=String(e),this.persistDuration(),n?.querySelectorAll(`.chip`).forEach(e=>{let t=Number(e.dataset.duration);e.classList.toggle(`chip--on`,t===this.durationSec)})}),this.root.querySelector(`#btn-start`)?.addEventListener(`click`,()=>{this.startRound()})}async startRound(){let e=this.deck;e&&(await f(),this.tiltDenied=!await l(),this.session=i(e.displayName,e.id,this.durationSec),this.playWords=b([...e.words]),this.playIndex=0,this.playDurationMs=this.durationSec*1e3,this.playStartedAt=performance.now(),this.lastTimerCeilSec=null,this.lastAdvanceAt=0,this.lastSubmittedPlayIndex=null,this.phase=`play`,this.stopPlayLoop(),this.tiltCleanup?.(),this.tiltCleanup=null,this.render())}mountPlay(){let e=this.deck,t=this.session;if(!e||!t)return;let n=this.tiltDenied?`<p class="play-banner" role="status">Motion permission not granted — use the buttons below.</p>`:`<p class="play-hint muted small">Tilt forward = correct · back = pass · or use buttons</p>`;this.root.innerHTML=`
      <div class="play">
        <header class="play-head">
          ${n}
          <div class="play-meta">
            <span class="muted small">${w(e.displayName)}</span>
            <span class="timer" id="play-timer" aria-live="polite">${S(this.durationSec)}</span>
          </div>
        </header>
      ...
      </div>`,this.wordEl=this.root.querySelector(`#play-word`),this.timeEl=this.root.querySelector(`#play-timer`),this.updateWordDisplay();let r=()=>this.advanceWord(`correct`),i=()=>this.advanceWord(`pass`);this.root.querySelector(`#btn-correct`)?.addEventListener(`click`,r),this.root.querySelector(`#btn-pass`)?.addEventListener(`click`,i),this.tiltCleanup=c(r,i,{cooldownMs:_,isActive:()=>this.phase===`play`});let a=()=>{if(this.phase!==`play`)return;let e=performance.now()-this.playStartedAt,t=this.playDurationMs-e;if(t<=0){this.endRound();return}this.timeEl&&(this.timeEl.textContent=S(t/1e3));let n=Math.max(0,Math.ceil(t/1e3));if(this.lastTimerCeilSec!==n){let e=this.lastTimerCeilSec;this.lastTimerCeilSec=n,e!==null&&n>=1&&n<=10&&t>0&&(this.reducedMotion?n===10&&h():h())}this.raf=requestAnimationFrame(a)};this.raf=requestAnimationFrame(a)}updateWordDisplay(){if(!this.wordEl)return;let e=this.playWords[this.playIndex];this.wordEl.textContent=e??``}advanceWord(e){if(this.phase!==`play`||!this.session)return;let t=performance.now();if(t-this.lastAdvanceAt<_)return;let n=this.playWords[this.playIndex];n&&(this.lastAdvanceAt=t,this.session.events.push({word:n,outcome:e}),this.lastSubmittedPlayIndex=this.playIndex,e===`correct`?p():m(),this.playIndex+=1,this.playIndex>=this.playWords.length&&this.deck&&(this.playWords=b([...this.deck.words]),this.playIndex=0,this.lastSubmittedPlayIndex=null),this.updateWordDisplay())}stopPlayLoop(){this.raf&&=(cancelAnimationFrame(this.raf),0)}endRound(){if(this.phase!==`play`)return;this.stopPlayLoop(),this.tiltCleanup?.(),this.tiltCleanup=null;let e=this.session,t=this.playWords[this.playIndex];e&&t!==void 0&&this.playIndex!==this.lastSubmittedPlayIndex&&e.events.push({word:t,outcome:`pass`}),this.phase=`ended`,this.wordEl=null,this.timeEl=null,this.render()}renderEnded(){let e=this.session;if(!e){this.root.innerHTML=`
        ...
        </main>`,this.root.querySelector(`#btn-again`)?.addEventListener(`click`,()=>{this.phase=`setup`,this.render()});return}let{correct:t,passed:n}=a(e),r=`${w(e.deckDisplayName)} · ${e.durationSec}s round`,i=e.events.map(e=>{let t=e.outcome===`correct`,n=t?`✓`:`✗`,r=t?`correct`:`passed`;return`<li class="result-row" aria-label="${w(`${e.word}, ${r}`)}">
          <span class="result-mark result-mark--${t?`ok`:`no`}" aria-hidden="true">${n}</span>
          <span class="result-word">${w(e.word)}</span>
      ...
      </div>`,this.root.querySelector(`#btn-again`)?.addEventListener(`click`,()=>{this.session=null,this.phase=`setup`,this.render()})}};function w(e){return e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`)}var T=document.querySelector(`#app`);if(!T)throw Error(`#app missing`);new C(T).boot();